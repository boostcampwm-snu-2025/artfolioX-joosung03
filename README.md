...
- Templates 탭(신규)
  - 포트폴리오용 템플릿을 사용자가 직접 저장할 수 있습니다(규칙 빌더: 카테고리/최소/최대).
  - 템플릿을 포트폴리오에 적용하고 준비도(충족/부족/초과)를 계산해 결과를 표시합니다.
  - 카테고리 규칙은 한글 기준으로 입력/저장하며, 서버는 한글/기존 영문 코드 모두 매칭하도록 정규화합니다.

- 공유 링크(읽기 전용 뷰어) + 코멘트 수집
  - 포트폴리오에서 공유 링크를 발급하고, `/share/:slug` 에서 읽기 전용 뷰어로 확인할 수 있습니다.
  - 공유 뷰에서 선생님 코멘트를 작성하면 서버에 저장되고 리스트로 표시됩니다.

- Works 캔버스 메모(작품별)
  - Works 페이지 우측 패널에 작품별 캔버스를 두고 펜/도형(박스)/텍스트/지우개로 메모할 수 있습니다.
